...
resource "null_resource" "bootstrap_instance" {
  ...
  provisioner "local-exec" {
    command = "scp -C ${path.module}/scripts/bootstrap.py ${local.secret_instance_credentials.username}@${local.ip_addr}:/opt/terraform/bootstrap.py && ssh -t ${local.secret_instance_credentials.username}@${local.ip_addr} \"cloud-init status --wait && python3 /opt/terraform/bootstrap.py\""
  }
}